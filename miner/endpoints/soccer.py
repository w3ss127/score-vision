    ...
    try:
        ...
        async for frame_number, frame in video_processor.stream_frames(video_path):
            ...
            if frame_number % 100 == 0:
                ...
                logger.info(
                    f"Processed {frame_number} frames in {elapsed:.1f}s ({fps:.2f} fps)"
                )
...
async def process_challenge(
    ...
    async with miner_lock:
        ...
        try:
            ...
            logger.info(
                f"Received challenge data: {json.dumps(challenge_data, indent=2)}"
            )
            ...
            try:
                tracking_data = await process_soccer_video(video_path, model_manager)
                ...
                logger.info(
                    f"Completed challenge {challenge_id} in {tracking_data['processing_time']:.2f} seconds"
                )

                output_dir = Path(__file__).parent.parent / "challenge_outputs"
            ...
            raise HTTPException(
                status_code=500, detail=f"Challenge processing error: {str(e)}"
            )
        finally:
